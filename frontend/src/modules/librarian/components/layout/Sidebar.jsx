    ...
    >
        ...
        <div className={cn('flex min-w-0 items-center', isCollapsed ? 'justify-center' : 'gap-2.5')}>
          ...
          {!isCollapsed && (
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Librarian Portal</p>
            </div>
          )}
        </div>
        {!isCollapsed && (
      ...
      <nav className="flex-1 overflow-y-auto px-2.5 py-3 space-y-1.5 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800" style={{ minHeight: 0 }}>
        {NAVIGATION_ITEMS.map((item) => {
          ...
          // Single Direct Item (e.g. Dashboard, Notifications)
          if (!item.children) {
            const isActive =
          ...
          return (
            <div key={item.title} className="space-y-1">
              <button
                ...
                title={isCollapsed ? item.title : undefined}
                className={cn(
                  'w-full flex items-center rounded-xl text-sm font-medium transition-all duration-150 select-none group',
        ...
        <div className="shrink-0 border-t border-slate-200 bg-white px-2.5 py-3 dark:border-slate-800 dark:bg-slate-950">
            ...
            {!isCollapsed && (
              <>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[10px] text-slate-500 dark:text-slate-400">Head Librarian</p>
                </div>
                <button
                  type="button"
          ...
          {isCollapsed && (
            <button
              type="button"
              onClick={handleLogout}
              className="mt-2 flex w-full items-center justify-center rounded-xl p-2 text-rose-500 transition-colors hover:bg-rose-50 dark:hover:bg-rose-500/10"
              aria-label="Logout"
              title="Sign out"
            >
              <LogOut className="h-4 w-4" />
