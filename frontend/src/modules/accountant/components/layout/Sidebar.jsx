    ...
    >
      ...
      <div className="flex items-center justify-between mb-8 px-2">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-violet-500/10 rounded-xl text-violet-405">
          ...
          {!isCollapsed && (
            ...
            >
              ...
              <span className="text-[10px] text-slate-550 font-bold tracking-wider uppercase mt-1 block">Account Desk</span>
            </motion.div>
          )}
        </div>
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="p-1.5 hover:bg-slate-900 rounded-lg text-slate-455 hover:text-white transition-colors"
        >
      ...
      {user && !isCollapsed && (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="text-[9px] text-slate-500 font-medium truncate block">{user.role}</span>
          </div>
        </motion.div>
      )}
      ...
      <nav className="flex-1 space-y-5">
        {Object.entries(categories).map(([category, items]) => (
          <div key={category} className="space-y-1">
            {!isCollapsed && (
              <span className="px-3.5 text-[9px] font-black tracking-widest text-slate-500 uppercase block mb-2 select-none font-sans">
                {category}
              </span>
            )}
            {items.map((item) => {
      ...
      <button
        ...
        className="flex items-center gap-3 px-3.5 py-3 text-xs font-semibold text-rose-455 hover:bg-rose-500/10 rounded-xl transition-colors w-full mt-6 select-none text-left"
      >
