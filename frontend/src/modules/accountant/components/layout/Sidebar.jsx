...
export const Sidebar = ({ isCollapsed, setIsCollapsed }) => {
  const { logout, user } = useAccountantAuth();
  ...
  return (
    <aside 
      className={cn(
        "hidden md:flex flex-col h-screen sticky top-0 bg-white dark:bg-slate-950 text-slate-600 dark:text-slate-300 border-r border-border dark:border-slate-900 px-4 py-6 overflow-y-auto no-scrollbar z-40 transition-all duration-300 shrink-0",
    ...
    >
      ...
      <div className={cn(
        ...
        <div className={cn("flex items-center", isCollapsed ? "justify-center" : "gap-3")}>
          <div className="p-2 bg-violet-500/10 rounded-xl text-violet-500 dark:text-violet-400">
          ...
          {!isCollapsed && (
            ...
            >
              ...
              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold tracking-wider uppercase mt-1 block">Account Desk</span>
            </motion.div>
          )}
        </div>
        <button
      ...
      {user && !isCollapsed && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-900/60 border border-border dark:border-slate-800/80 rounded-2xl mb-6 shrink-0"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="text-[9px] text-slate-400 dark:text-slate-500 font-medium truncate block">{user.role}</span>
          </div>
        </motion.div>
      )}
      ...
      <nav className="flex-1 space-y-5">
        {Object.entries(categories).map(([category, items]) => (
          <div key={category} className="space-y-1">
            {!isCollapsed && (
              <span className="px-3.5 text-[9px] font-black tracking-widest text-slate-400 dark:text-slate-500 uppercase block mb-2 select-none font-sans">
                {category}
              </span>
            )}
            {items.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  className={cn(
                    ...
                    isActive 
                      ? "bg-violet-600 text-white shadow-lg shadow-violet-600/15 scale-[1.01]" 
