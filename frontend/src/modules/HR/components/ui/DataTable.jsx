...
export const DataTable = ({ 
  ...
  onRowClick,
  loading = false,
  emptyMessage = "No matching HRMS employee files found."
}) => {
  ...
  return (
    <div className="space-y-4 text-xs font-semibold">
        ...
        <div className="flex items-center gap-2 flex-1 max-w-md">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white pl-9 pr-4 py-2.5 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-850 focus:border-rose-600 focus:ring-1 focus:ring-rose-600 focus:outline-none"
            />
          </div>
        ...
        <div className="flex items-center gap-2.5 self-end md:self-auto">
          ...
          <div className="relative">
            <button
              ...
              className="px-3.5 py-2.5 bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-850 text-slate-500 rounded-xl text-xs font-bold hover:bg-slate-100 flex items-center gap-1.5"
            >
              <Eye className="w-3.5 h-3.5" />
              <span>Columns</span>
            </button>
            {showColumnToggle && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-202 dark:border-slate-850 rounded-2xl p-3 shadow-xl z-20 space-y-1.5">
                  ...
                  {columns.map(c => (
                    <label key={c.key} className="flex items-center gap-2 px-1 py-0.5 cursor-pointer text-xs font-semibold">
                      <input
                        ...
                        className="rounded text-rose-600 border-slate-350 w-3.5 h-3.5 focus:ring-rose-600"
                      />
          ...
          {/* CSV Export */}
          <button
            onClick={() => exportToCSV(processedData, `hrms_records_${Date.now()}.csv`)}
            className="px-3.5 py-2.5 bg-rose-600 hover:bg-rose-750 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-sm"
          >
      ...
      {showFilterDrawer && filterOptions.length > 0 && (
        <div className="bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-850 rounded-3xl p-4 grid grid-cols-2 md:grid-cols-4 gap-3.5">
          {filterOptions.map((filter) => (
            <div key={filter.key} className="space-y-1">
              ...
              <select
                ...
                className="w-full bg-white dark:bg-slate-900 border text-xs font-bold rounded-xl px-2.5 py-2 focus:ring-1 focus:ring-rose-600"
              >
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden">
        <div className="overflow-x-auto w-full">
          <table className="w-full text-left border-collapse font-sans">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-955 border-b border-slate-100 dark:border-slate-850">
                {bulkActions.length > 0 && (
                  <th className="px-6 py-4 w-10 shrink-0">
                    <input
                      ...
                      className="rounded text-rose-600 border-slate-350 focus:ring-rose-600 w-4 h-4 cursor-pointer"
                    />
                  </th>
                )}
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      {col.sortable && sortConfig && sortConfig.key === col.key && (
                        sortConfig.direction === 'ascending' ? <ChevronUp className="w-3 h-3 text-rose-650" /> : <ChevronDown className="w-3 h-3 text-rose-650" />
                      )}
                    </div>
                  </th>
                ))}
                ...
                paginatedData.map((row) => {
                  ...
                  return (
                    <tr 
                      ...
                      className={`hover:bg-slate-50/50 dark:hover:bg-slate-850/20 transition-all ${onRowClick ? 'cursor-pointer' : ''} ${isSelected ? 'bg-rose-50/10 dark:bg-rose-955/10' : ''}`}
                    >
                      {bulkActions.length > 0 && (
                      ...
                      {activeColumns.map((col) => (
                        <td key={col.key} className="px-6 py-3.5 text-xs font-semibold text-slate-805 dark:text-slate-200">
        ...
        <div className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-slate-100 dark:border-slate-850 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-50 dark:bg-slate-955 border border-slate-202 dark:border-slate-850 text-xs font-bold rounded-lg px-2 py-1 focus:ring-1 focus:ring-rose-650"
            >
              {[5, 10, 20, 50].map(sz => (
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="p-1.5 border dark:border-slate-800 rounded-lg bg-slate-50 dark:bg-slate-955 text-slate-500 disabled:opacity-40 hover:bg-slate-100"
            >
                ...
                <button
                  ...
                  className={`w-7 h-7 rounded-lg text-xs font-bold flex items-center justify-center transition-all ${
                    isCurrent 
                      ...
                      : 'border dark:border-slate-800 bg-slate-50 dark:bg-slate-955 text-slate-505 hover:bg-slate-100'
                  }`}
                >
                  {p}
                </button>
      ...
      {selectedIds.size > 0 && bulkActions.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 border border-slate-800 text-white px-5 py-3 rounded-2xl shadow-xl flex items-center gap-4 z-40 transition-all select-none">
          <span className="text-xs font-bold text-rose-450">{selectedIds.size} file(s) selected</span>
