  ...
  return (
    <header className="sticky top-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-805 h-16 flex items-center justify-between px-6 z-30 select-none">
      <div className="flex items-center gap-4 flex-1">
        <button
          onClick={onMenuClick}
          className="p-1.5 hover:bg-slate-105 dark:hover:bg-slate-800 rounded-lg text-slate-500 md:hidden transition-colors"
        >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute top-1.5 right-1.5 bg-rose-600 border border-white dark:border-slate-900 rounded-full w-2.5 h-2.5 animate-pulse"></span>
            )}
          </button>
          ...
          <AnimatePresence>
            {showNotifications && (
              <motion.div
                ...
                className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-855 rounded-2xl shadow-xl z-50 overflow-hidden text-left"
              >
                <div className="px-4 py-3 bg-slate-50 dark:bg-slate-955 border-b border-slate-100 dark:border-slate-850 flex items-center justify-between">
                  <span className="text-[11px] font-black text-slate-505 uppercase tracking-wider">Alert Feed ({unreadCount})</span>
                  {unreadCount > 0 && (
                    <button onClick={markAllAsRead} className="text-[10px] font-bold text-rose-600 dark:text-rose-400 hover:underline">
                      Mark all read
                    </button>
                  )}
                </div>
                <div className="max-h-68 overflow-y-auto no-scrollbar divide-y divide-slate-100 dark:divide-slate-850/50">
                  {notifications.slice(0, 5).map(item => (
                    <div 
                      ...
                      className={`p-3.5 hover:bg-slate-50/50 dark:hover:bg-slate-955/40 transition-colors cursor-pointer ${!item.read ? 'bg-rose-500/5' : ''}`}
                    >
                ...
                <div className="p-2 border-t text-center">
                  <Link 
                    ...
                    className="text-[11px] font-extrabold text-rose-600 dark:text-rose-400 block hover:underline"
                  >
                    View All Alerts
                  </Link>
                </div>
            ...
            <AnimatePresence>
              {showUserDropdown && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10, scale: 0.95 }}
                  transition={{ duration: 0.12 }}
                  className="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-900 border border-slate-202 dark:border-slate-850 rounded-2xl shadow-xl z-50 p-2 space-y-1"
                >
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-slate-550 hover:text-slate-800 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-950 rounded-xl"
                  >
