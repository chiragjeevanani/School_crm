...
import { LogOut, ChevronLeft, ChevronRight, Bus } from 'lucide-react';
        ...
        <div className={cn('flex min-w-0 items-center', isCollapsed ? 'justify-center' : 'gap-2.5')}>
          ...
          {!isCollapsed && (
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400">Transport Portal</p>
            </div>
          )}
        </div>
        {!isCollapsed && (
          ...
          >
            <div className="h-8 w-8 rounded-full bg-indigo-600/10 text-indigo-600 dark:bg-indigo-950/80 dark:text-indigo-400 flex items-center justify-center font-bold text-xs shrink-0 border border-indigo-200 dark:border-indigo-800">
              {user.photo || user.photoUrl ? (
                <img src={user.photo || user.photoUrl} alt={user.name} className="h-full w-full rounded-full object-cover" />
              ) : (
            ...
            {!isCollapsed && (
              <>
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[10px] text-slate-500 dark:text-slate-400">{user.role || 'Fleet Manager'}</p>
                </div>
                <button
                  type="button"
