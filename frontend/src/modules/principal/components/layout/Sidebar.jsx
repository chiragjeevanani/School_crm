        ...
        <div className={cn("flex items-center", isCollapsed ? "justify-center" : "gap-3")}>
          ...
          {!isCollapsed && (
            ...
            >
              ...
              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold tracking-wider uppercase mt-1 block">Principal Portal</span>
            </motion.div>
          )}
        </div>
        <button
      ...
      <nav className="flex-1 space-y-5">
        {Object.entries(categories).map(([category, items]) => (
          <div key={category} className="space-y-1">
            {!isCollapsed && (
              <span className="px-3.5 text-[9px] font-black tracking-widest text-slate-400 dark:text-slate-500 uppercase block mb-2 select-none">
                {category}
              </span>
            )}
            {items.map((item) => {
